...
output_call (struct cb_call *p)
{
	...
	if (CB_REFERENCE_P (p->name)
	 ...
	 && CB_FIELD (CB_REFERENCE (p->name)->value)->usage
		...
		/* Static link */
		char * system_call = NULL; //// not defined for 1547 merge
		if (system_call) {
...
output_report_sum_counters (const int top, struct cb_field *f, struct cb_report *r)
{
	...
	if(f->report_sum_list == NULL)
		return;
	if(f->report_flag & COB_REPORT_SUM_EMITTED)	/* already emitted? */
		return;
...
output_internal_function (struct cb_program *prog, cb_tree parameter_list)
{
	...
	for (i = 0, l = parameter_list; l; l = CB_CHAIN (l), i++) {
		output_prefix ();
		output_line ("if(module->module_num_params < %d)",i+1);
		output_indent_level += 2;
		output_prefix ();
		pickup_param (l, i, 2);
		output_indent_level -= 2;
...
#if 1 //// reportwriter //// 1547: corrupted merge?  prog and entry_name not defined
...
codegen (struct cb_program *prog, const int subsequent_call)
{
	...
	if (0 && local_field_cache) { //// brought in from reportwriter.  Seems to generate invalid local variables (test #2)
		...
		if (prog->report_storage) {
			for (l = prog->report_list; l; l = CB_CHAIN (l)) {
				rep = CB_REPORT(CB_VALUE(l));
				for (struct cb_field *fld=rep->records; fld; fld=fld->sister) {
