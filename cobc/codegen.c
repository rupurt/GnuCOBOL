		...
		case CB_CALL_BY_CONTENT:
			if (CB_CAST_P (x)) {
				...
				if (CB_NUMERIC_LITERAL_P (x)) {
					...
					if (cb_binary_byteorder == CB_BYTEORDER_BIG_ENDIAN) {
						output_prefix ();
						output("content_%s%u.flags |= COB_FLAG_BINARY_SWAP;\n",
									CB_PREFIX_ATTR, n);
						output_prefix ();
						output ("content_%u.dataint = ", n);
						output ("(unsigned int)COB_BSWAP_32(");
...
{
	/* shouldn't happen and if: should COBC_ABORT */
	if (p == NULL) {
		return;
	}
...
output_report_sum_counters (int top, struct cb_field *f, struct cb_report *r)
{
	...
	for(p=f; p; p = p->parent) {
		if(p->report_control) {
			output_local("&%s%d_%d,",CB_PREFIX_REPORT_CONTROL,r_ctl_id,cb_code_field(p->report_control)->id);
			break;
...
static void
pickup_param (cb_tree l, int i, int setnull)
{
	char	wrk[64];
	cb_tree	x;
	FILE	*savetarget;
...
output_internal_function (struct cb_program *prog, cb_tree parameter_list)
{
	...
	&& (anyseen || valueseen)) {
		if (anyseen) {
			output_line ("/* Initialize ANY LENGTH %sparameters */",valueseen?"and BY VALUE ":"");
		} else {
			output_line ("/* Initialize BY VALUE parameters */");
		}
		name_hash = cob_get_name_hash (prog->orig_program_id);
	...
	if (cb_list_length (prog->entry_list) > 1) {
		...
		for (i = 0, l = prog->entry_list; l; l = CB_CHAIN (l)) {
			...
			output_indent_level += 4;
			if (using_list) {
				output_line ("/* Initialize %d parameters for '%s' */",
				...
				output_line ("/* No parameters for '%s' */",
