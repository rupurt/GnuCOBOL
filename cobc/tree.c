...
cb_build_perform_varying (cb_tree name, cb_tree from, cb_tree by, cb_tree until)
{
	...
	if (warningopt) {
		cb_source_line--;
		if (until == cb_false) {
			cb_warning (_("PERFORM FOREVER since UNTIL is always FALSE"));
		} else if (until == cb_true) {
			if (after_until) {
				cb_warning (_("PERFORM ONCE since UNTIL is always TRUE"));
			} else {
				cb_warning (_("PERFORM NEVER since UNTIL is always TRUE"));
			}
		}
		cb_source_line++;
	}
			 ...
			 && !f->flag_any_length) {
				if (!(f->pic 
				 && (f->pic->category == CB_CATEGORY_NATIONAL
				  || f->pic->category == CB_CATEGORY_NATIONAL_EDITED))) 
